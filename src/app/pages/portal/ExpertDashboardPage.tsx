...
import {
  Calendar, Users, FileText, Video, Clock, TrendingUp, Clock3,
} from "lucide-react";
...
export default function ExpertDashboardPage() {
  ...
  useEffect(() => { loadDashboardData(); }, [accessToken]);
    ...
    <ExpertLayout title="Tableau de bord">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-sm text-[#1A1A1A]/60 mt-1">Voici un aperçu de votre activité</p>
        </div>

        {/* Stats Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {[
            { icon: <Calendar className="w-6 h-6 text-[#A68B6F]" />, bg: "bg-[#A68B6F]/10", label: "Aujourd'hui", value: stats.todayConsultations, sub: "Consultations" },
            { icon: <TrendingUp className="w-6 h-6 text-[#C1694F]" />, bg: "bg-[#C1694F]/10", label: "Cette semaine", value: stats.weekConsultations, sub: "Consultations" },
            { icon: <Users className="w-6 h-6 text-blue-500" />, bg: "bg-blue-500/10", label: "Total", value: stats.totalPatients, sub: "Patients" },
            { icon: <FileText className="w-6 h-6 text-green-500" />, bg: "bg-green-500/10", label: "Documents", value: stats.pendingDocuments, sub: "En attente" },
          ].map((stat, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-xs font-semibold uppercase tracking-wider text-[#1A1A1A]/60">{stat.label}</span>
              </div>
              <p className="text-3xl font-serif text-[#1A1A1A] mb-1">{stat.value}</p>
              <p className="text-sm text-[#1A1A1A]/60">{stat.sub}</p>
            </motion.div>
          ))}
        </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            {/* ✅ Disponibilités remplace Calendrier */}
            <Link to="/expert/availability"
              ...
              <div className="w-14 h-14 bg-[#C1694F]/10 rounded-full flex items-center justify-center group-hover:bg-[#C1694F] transition-colors">
                <Clock3 className="w-7 h-7 text-[#C1694F] group-hover:text-white transition-colors" />
              </div>
              <span className="text-sm font-medium text-[#1A1A1A]">Disponibilités</span>
            </Link>

            <Link to="/expert/documents"
              className="flex flex-col items-center gap-3 p-4 hover:bg-[#F5F1ED] rounded-xl transition-colors group">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}
            ...
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-serif text-[#1A1A1A]">Consultations à venir</h2>
              <Link to="/expert/agenda" className="text-sm text-[#A68B6F] hover:text-[#8A7159] transition-colors">Voir tout</Link>
            </div>
            {upcomingConsultations.length === 0 ? (
              <div className="text-center py-8">
                <Clock className="w-12 h-12 text-[#1A1A1A]/20 mx-auto mb-3" />
                <p className="text-sm text-[#1A1A1A]/60">Aucune consultation programmée</p>
              </div>
            ) : (
              <div className="space-y-4">
                {upcomingConsultations.map((c) => (
                  <div key={c.id} className="flex items-center justify-between p-4 bg-[#F5F1ED] rounded-xl">
                    ...
                    <div className="text-right">
                      ...
                      <Link to={`/expert/consultation-room/${c.id}`} className="text-xs text-[#A68B6F] hover:text-[#8A7159] transition-colors">Rejoindre</Link>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}
            className="bg-white rounded-2xl p-6 border border-[#D4C5B9]">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-serif text-[#1A1A1A]">Patients récents</h2>
              <Link to="/expert/patients" className="text-sm text-[#A68B6F] hover:text-[#8A7159] transition-colors">Voir tout</Link>
            </div>
            {recentPatients.length === 0 ? (
              <div className="text-center py-8">
                <Users className="w-12 h-12 text-[#1A1A1A]/20 mx-auto mb-3" />
                <p className="text-sm text-[#1A1A1A]/60">Aucun patient pour le moment</p>
              </div>
            ) : (
              <div className="space-y-3">
                {recentPatients.map((patient) => (
                  <Link key={patient.id} to={`/expert/patients/${patient.id}`}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-[#1A1A1A]/60">{patient.consultationsCount} consultation{patient.consultationsCount > 1 ? "s" : ""}</p>
                      </div>
                    </div>
